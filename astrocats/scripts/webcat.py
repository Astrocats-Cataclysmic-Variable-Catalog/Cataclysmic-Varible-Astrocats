...
linkdir = "https://" + moduleurl + "/" + modulename + "/"
...
with open('astrocats/' + moduledir + '/html/sitemap-template.xml', 'r') as f:
...
csvpages = [[
    "# name", "aliases", "max apparent mag", "max mag date", "claimed type",
    "redshift", "redshift kind", "ra", "dec", "# of photometric obs.", "URL"
]]
sourcedict = {}
nophoto = []
lcspye = []
...
for fcnt, eventfile in enumerate(tq(sorted(files, key=lambda s: s.lower()))):
    ...
    if xrayavail and dohtml and args.writehtml:
        ...
        hover = HoverTool(tooltips=tt, renderers=ttglyphs)
        p4.add_tools(hover)
# --------below is removed from the original script due to the excessice time it takes to download visual files from sdss
#    hasimage = False
#    skyhtml = ''
...
#            ncols=2,
#            toolbar_location=None) #gridplot could be used as a substitude
#
...
#            f.write(jsonstring)
    ...
    if not args.boneyard:
        ...
        with open(outdir + htmldir + modulename + 'pages.csv' + testsuffix,
                  'w') as f:
        ...
        with open(outdir + htmldir + 'info-snippets/' + modulename +
                  'count.html' + testsuffix, 'w') as f:
        ...
        with open(outdir + htmldir + 'sitemap.xml', 'w') as f:
            ...
            for key in catalog.keys():
                sitemaplocs = sitemaplocs + "  <url>\n    <loc>https://" + moduleurl + "/" + modulename + "/" + \
                    key + "</loc>\n  </url>\n"
    ...
    with open(outdir + catprefix + '.min.json'+ testsuffix, 'rb') as f_in, gzip.open(
            outdir + catprefix + '.min.json.gz', 'wb') as f_out:
        shutil.copyfileobj(f_in, f_out)

    names = OrderedDict()
    for ev in catalog:
        names[ev['name']] = [x['value']
                             for x in ev.get('alias', [{'value': ev['name']}])]
